...
## Goal
Allow users to open a song from LyricLoop directly in Spotify, Apple Music, or YouTube using deep links, without in-app playback control or OAuth login.
...
- Background playback managed by LyricLoop.
- Hard real-time lyrics sync with external player state.

## User Experience
1. User opens a song in LyricLoop.
